...
public class ObjModel
{
    ...
    public void bindTextures(Context context, GL10 gl)
    {
        ...
        try {
            InputStream is = context.getAssets().open(mTextureName);
            bitmap = BitmapFactory.decodeStream(is);
            if (bitmap == null) {
                Log.v("ObjModel", "err decoding bitmap!");
            }
    ...
    public static ObjModel loadFromAssets(Context context, String model_name, String texture_name) throws IOException
    {
        InputStream is = context.getAssets().open(model_name);
        ObjModel obj = ObjModel.loadFromStream(is, texture_name);
        is.close();
        return obj;
    }
    ...
    private static class Model 
    {
        ...
        public void fill(ArrayList<Face> faces, boolean has_tex, boolean has_normals)
        {
            ...
            this.v_size = f_len * 3;
            this.v = ObjModel.floatBuffer(this.v_size*3);

            if (has_tex)
                this.vt = ObjModel.floatBuffer(this.v_size*2);
            if (has_normals)
                this.vn = ObjModel.floatBuffer(this.v_size*3);
    ...
    public static FloatBuffer floatBuffer(int num_floats)
    {
